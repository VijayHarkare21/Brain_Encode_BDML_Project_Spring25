...
embedding spaces, optimized to avoid OOM by using condensed RDMs.

Usage
=====
python rsa_cca_group_analysis_condensed.py \
    --mode both \
...
def load_eeg_image_data(directory: str) -> Dict[str, dict]:
    ...
    for fname in sorted(os.listdir(directory)):
        ...
        if 'test' in fname.lower(): continue
...
def embed_texts(
    ...
    pm = {'bert':'bert-base-uncased','roberta':'roberta-base'}
    tok = AutoTokenizer.from_pretrained(pm[model_name.lower()], cache_dir=cache_dir)
    model = AutoModel.from_pretrained(pm[model_name.lower()], cache_dir=cache_dir).to(device).eval()
    embs = []
...
def embed_images(
    ...
    if model_name.lower() == 'vit':
        ...
        for i in range(0, len(paths), batch_size):
            ...
            embs.append(vecs.cpu().numpy().astype(np.float32))
        result = np.vstack(embs)
        np.savez(cache_path, embs=result)
        return result
    else:
...
def _aggregate(by_key: defaultdict, how: str, zscore: bool
    ...
    keys = sorted(by_key)
    if how == 'average':
        mat = np.vstack([np.mean(by_key[k],0) for k in keys]).astype(np.float32)
...
def aggregate_eeg_image(
    img_data: Dict[str, dict], eeg_type: str,
    how: str='average', zscore: bool=True
) -> Tuple[List[str], np.ndarray, Optional[List[int]]]:
    by_path = defaultdict(list)
    for subj_arr in img_data.values():
        for p, e in zip(subj_arr['img_paths'], random_utils.create_random_array(subj_arr[f'embeds_{eeg_type}'].shape)):
            by_path[p].append(e)
    return _aggregate(by_path, how, zscore)


def aggregate_eeg_text(
    txt_data: Dict[str, List[dict]], eeg_type: str,
    how: str='average', zscore: bool=True
) -> Tuple[List[str], np.ndarray, Optional[List[int]]]:
    ...
    for sid in subj_ids:
        ...
        per_subj.append(seen)
    common = sorted(set.intersection(*per_subj))
    if not common:
        raise ValueError("No sentences are common across all subjects.")
    subj_sent = {sid: {} for sid in subj_ids}
    for sid in subj_ids:
        by_sent = defaultdict(list)
        for t in txt_data[sid]:
            if t and f'embeds_{eeg_type}' in t and t['content'] in common:
                by_sent[t['content']].append(random_utils.create_random_array(t[f"embeds_{eeg_type}"].shape if isinstance(t[f"embeds_{eeg_type}"], np.ndarray) else t[f"embeds_{eeg_type}"].numpy().shape))
        for sent in common:
            subj_sent[sid][sent] = np.mean(by_sent[sent], axis=0)
    if how == 'average':
        mat = np.vstack([
            np.mean([subj_sent[sid][sent] for sid in subj_ids], axis=0)
            for sent in common
        ]).astype(np.float32)
        if zscore: mat = _zscore_rows(mat)
    ...
    for subj_idx, sid in enumerate(subj_ids):
        for sent in common:
            rows.append(subj_sent[sid][sent])
...
def rsa_cca_pair(
    ...
    # Compute condensed RDMs directly to save memory
    rdm_e_vec = pdist(eeg_emb, metric='correlation')
    rdm_n_vec = pdist(nat_emb, metric='correlation')
    rho, pval = spearmanr(rdm_e_vec, rdm_n_vec)
    ...
    return float(rho), float(pval), corrs, cca


def main():
    p = argparse.ArgumentParser(description="Group-level RSA/CCA with condensed RDM")
    ...
    p.add_argument("--figures_dir", type=str, default="figs")
    ...
    p.add_argument(
        ...
        type=float,
        default=0.05,
        help="when using stack mode, only sample this fraction of items (0–1)",
    )
    args = p.parse_args()
    ...
    if args.mode in ("image","both"):
        ...
        for et in args.eeg_embedding_types:
            ...
            if args.subject_aggregate == "stack" and args.stack_fraction < 1.0:
                ...
                sampled = list(np.random.choice(all_paths, size=k, replace=False))
                filt_data = {}
                for subj, arr in img_data.items():
                    ...
                    for et2 in args.eeg_embedding_types:
                        entry[f"embeds_{et2}"] = arr[f"embeds_{et2}"][mask]
                    filt_data[subj] = entry
            ...
            for m in args.image_models:
                ...
                rho, pval, corrs, cca_model = rsa_cca_pair(
                    eeg_mat, nat_emb, paths,
                    f"IMG_{et}_{m}_{args.subject_aggregate}",
                    args.figures_dir, args.cca_components
                )

                # train/test pipeline
                idxs = np.arange(eeg_mat.shape[0])
                ...
                # test-set RSA via condensed vectors
                rdm_pred = pdist(eeg_pred, metric='correlation')
                rdm_true = pdist(eeg_mat[te], metric='correlation')
                rsa_test, _ = spearmanr(rdm_pred, rdm_true)

                print(f"[PIPE] IMG_{et}_{m} test MSE={mse:.4f}, R2={r2:.4f}, RSA={rsa_test:.4f}")
                results.append({
                    "analysis":   "image",
                    "eeg_type":   et,
                    "model":      m,
        ...
        if args.run_ensemble:
            # ensemble logic unchanged, but uses pdist for test RSA
            if args.subject_aggregate == "stack" and args.stack_fraction < 1.0:
                mat1 = aggregate_eeg_image(filt_data, args.eeg_embedding_types[0], "stack", zscore)[1]
                mat2 = aggregate_eeg_image(filt_data, args.eeg_embedding_types[1], "stack", zscore)[1]
            else:
                mat1 = aggregate_eeg_image(img_data, args.eeg_embedding_types[0], args.subject_aggregate, zscore)[1]
                mat2 = aggregate_eeg_image(img_data, args.eeg_embedding_types[1], args.subject_aggregate, zscore)[1]
            ...
            for m in args.image_models:
                ...
                rho, pval, corrs, cca_model = rsa_cca_pair(
                    eeg_ens, nat_emb, paths,
                    f"IMG_ensemble_{m}_{args.subject_aggregate}",
                    args.figures_dir, args.cca_components
                )

                tr, te = train_test_split(np.arange(eeg_ens.shape[0]), test_size=args.test_size, random_state=42)
                W = cca_model.y_weights_.dot(cca_model.x_weights_.T)
                eeg_pred = nat_emb[te].dot(W)
                mse      = mean_squared_error(eeg_ens[te], eeg_pred)
                r2       = r2_score(eeg_ens[te], eeg_pred, multioutput='uniform_average')

                rdm_pred = pdist(eeg_pred, metric='correlation')
                rdm_true = pdist(eeg_ens[te], metric='correlation')
                rsa_test, _ = spearmanr(rdm_pred, rdm_true)

                print(f"[PIPE] IMG_ensemble_{m} test MSE={mse:.4f}, R2={r2:.4f}, RSA={rsa_test:.4f}")
                results.append({
                    "analysis":   "image",
                    "eeg_type":   "ensemble",
                    "model":      m,
    ...
    if args.mode in ("text","both"):
        ...
        n_sub = len(txt_data)
        for et in args.eeg_embedding_types:
            keys, eeg_mat, _ = aggregate_eeg_text(txt_data, et, args.subject_aggregate, zscore)
            for m in args.text_models:
                nat_base = embed_texts(keys, m, dev, args.cache_dir, args.batch_size, args.embed_cache_dir)
                if zscore: nat_base = _zscore_rows(nat_base)
                nat_emb = np.vstack([nat_base for _ in range(n_sub)]) if args.subject_aggregate=="stack" else nat_base

                rho, pval, corrs, cca_model = rsa_cca_pair(
                    eeg_mat, nat_emb, keys,
                    f"TXT_{et}_{m}_{args.subject_aggregate}",
                    args.figures_dir, args.cca_components
                )

                tr, te = train_test_split(np.arange(eeg_mat.shape[0]), test_size=args.test_size, random_state=42)
                W = cca_model.y_weights_.dot(cca_model.x_weights_.T)
                eeg_pred = nat_emb[te].dot(W)
                mse = mean_squared_error(eeg_mat[te], eeg_pred)
                r2 = r2_score(eeg_mat[te], eeg_pred, multioutput='uniform_average')
                ...
                print(f"[PIPE] TXT_{et}_{m} test MSE={mse:.4f}, R2={r2:.4f}, RSA={rsa_test:.4f}")
                results.append({"analysis": "text", "eeg_type": et, "model": m,
                                 "rsa_rho": rho, "rsa_p": pval, "cca_corrs": corrs,
                                 "test_mse": mse, "test_r2": r2, "test_rsa": rsa_test})
        ...
        if args.run_ensemble:
            ...
            for m in args.text_models:
                ...
                rho, pval, corrs, cca_model = rsa_cca_pair(
                    eeg_ens, nat_emb, keys,
                    f"TXT_ensemble_{m}_{args.subject_aggregate}",
                    args.figures_dir, args.cca_components
                )
                ...
                print(f"[PIPE] TXT_ensemble_{m} test MSE={mse:.4f}, R2={r2:.4f}, RSA={rsa_test:.4f}")
                results.append({"analysis": "text", "eeg_type": "ensemble", "model": m,
                                 "rsa_rho": rho, "rsa_p": pval, "cca_corrs": corrs,
                                 "test_mse": mse, "test_r2": r2, "test_rsa": rsa_test})
