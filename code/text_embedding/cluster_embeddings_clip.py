...
def cluster_embeddings(embeddings, max_clusters=100, plot=True):
    ...
    if plot:
        ...
        # Apply same transformation to cluster centers as that to embeddings
        tsne_embeddings = tsne.fit_transform(scaled_embeddings)
        # tsne_centers = tsne.fit_transform(scaled_centers)
        ...
        plt.legend()
        plt.savefig('clustered_embeddings_clip.png')
        plt.show()

    return kmeans, silhouette_avg

data = np.load(r"D:\Vijay\NYU\Spring_25\BDMLS\Project\code\text_embedding\text_embeddings.npy", allow_pickle=True)

embeddings = np.array(data)
print(f"Shape of embeddings: {embeddings.shape}")
# exit(0)
# Perform clustering on the embeddings
kmeans, silhouette_avg = cluster_embeddings(embeddings, max_clusters=20, plot=True)